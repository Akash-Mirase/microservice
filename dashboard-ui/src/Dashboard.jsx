...
function statusColor (s) {
  return s === 'UP' ? C.green : s === 'DOWN' ? C.red : C.amber
}
function sevColor (n) {
  const v = parseFloat(n) || 0
  return v > 70 ? C.red : v > 40 ? C.amber : C.green
}
function riskColor (r) {
  return (
    { CRITICAL: C.red, HIGH: C.amber, MEDIUM: C.accent, LOW: C.green }[r] ||
    C.muted
  )
}
function aColor (t) {
  return (
    {
      MEMORY_LEAK: C.purple,
      CPU_LEAK: C.amber,
      CASCADE_FAILURE: C.red,
      CACHE_POISON: C.accent,
      RESOURCE_EXHAUSTION: C.red
    }[t] || C.muted
  )
}
function fmt (v, d = 1) {
...
function ProgressBar ({ pct, color }) {
  const c = parseFloat(pct) || 0,
    col = c > 85 ? C.red : c > 60 ? C.amber : color || C.green
  return (
    <div
      style={{
        height: 4,
...
function StatCard ({ label, value, color, sub }) {
  return (
    <div style={pnl()}>
      ...
      {sub && (
        <p style={{ margin: '4px 0 0', fontSize: 10, color: C.muted }}>{sub}</p>
      )}
    </div>
  )
}
...
export default function Dashboard () {
  ...
  const fetchAll = useCallback(async () => {
    ...
    if (res[0].status === 'fulfilled') {
      ...
      setHealth(Array.isArray(data) ? data : data.services || [])
    }
  ...
  useEffect(() => {
    ...
    socket.on('event-update', data =>
      setLiveEvents(p => [data, ...p].slice(0, 100))
    )
  ...
  const live = services.length > 0 ? services : health
  ...
  if (loading)
    return (
      ...
      >
        ...
        <p
          style={{ color: C.accent, fontSize: 13, letterSpacing: 3, margin: 0 }}
        >
  ...
  return (
    ...
    >
      ...
      <header
        style={{
          background: `linear-gradient(90deg,#080d18 0%,#0d1e35 50%,#080d18 100%)`,
          borderBottom: `1px solid ${C.border}`,
          padding: '16px 24px',
          display: 'flex',
          alignItems: 'center',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            ...
            <p style={{ margin: '2px 0 0', fontSize: 11, color: C.muted }}>
              Detection · Diagnosis · Healing · Prediction
            </p>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          {incident?.active && (
            <div style={badge(C.red)}>
              🔴 INCIDENT: {incident.service} › {incident.stage}
            </div>
          )}
          <div
            style={{
      ...
      <div style={{ padding: 24 }}>
        ...
        {tab === 'healing' && (
          <HealingTab diagnoses={diagnoses} services={live} />
        )}
    ...
    <div>
      ...
      >
        <StatCard
          ...
          color={up === services.length ? C.green : C.amber}
          sub={`${services.length - up} degraded`}
        />
        <StatCard
          label='Critical Anomalies'
          value={crit}
          color={crit > 0 ? C.red : C.green}
          sub='severity > 70%'
        />
        <StatCard
          label='Predictive Alerts'
          value={alertCrit}
          color={alertCrit > 0 ? C.amber : C.green}
          sub='CRITICAL risk'
        />
        <StatCard
          label='Uptime'
          value={sla ? `${parseFloat(sla.uptime).toFixed(2)}%` : '—'}
          color={C.accent}
          sub='current window'
        />
        <StatCard
        ...
        <StatCard
          label='Events'
          value={sla?.incidents ?? liveEvents.length}
          color={C.muted}
          sub='logged'
        />
      </div>
      ...
      >
                  ...
                  <div key={name}>
                    ...
                    >
                      <div>
                        ...
                        <ProgressBar pct={s.cpu} color={C.accent} />
                      </div>
                      <div>
                        <div style={{ fontSize: 10, color: C.muted }}>
                          MEM {fmt(s.memory)}%
                        </div>
                        <ProgressBar pct={s.memory} color={C.purple} />
                      </div>
                    </div>
                  </div>
                )
        ...
        <div style={pnl()}>
          ...
          {heatmap.length === 0 && diagnoses.length === 0 ? (
            <p
              style={{
                textAlign: 'center',
                color: C.muted,
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              {rootCause.anomaly_type && (
                <div>
                  ...
                  <span style={badge(aColor(rootCause.anomaly_type))}>
                    {rootCause.anomaly_type}
                  </span>
                </div>
              )}
              ...
              {rootCause.confidence != null && (
                <div>
                  ...
                  <ProgressBar
                    pct={(rootCause.confidence || 0) * 100}
                    color={C.accent}
                  />
                  <p
                    style={{ fontSize: 10, color: C.accent, margin: '3px 0 0' }}
                  >
          ...
          <div style={pnl()}>
            ...
            >
              {[
                {
                  label: 'Uptime',
                  value: `${parseFloat(sla.uptime).toFixed(3)}%`,
                  color: parseFloat(sla.uptime) > 99 ? C.green : C.amber
                },
                { label: 'MTTR', value: sla.mttr, color: C.accent },
                { label: 'MTBF', value: sla.mtbf, color: C.muted },
                { label: 'Incidents', value: sla.incidents, color: C.muted }
              ].map(item => (
      ...
      {(timeline.length > 0 || liveEvents.length > 0) && (
        <div style={{ ...pnl(), marginTop: 20 }}>
          <SectionTitle
            ...
            extra={<span style={badge(C.green)}>REALTIME</span>}
          />
          <div style={logBox}>
            {[...liveEvents, ...timeline].slice(0, 40).map((ev, i) => (
              <div
                key={ev.id || i}
                style={{ display: 'flex', gap: 12, marginBottom: 2 }}
              >
                <span style={{ color: C.muted, minWidth: 78, fontSize: 10 }}>
                ...
                <span
                  style={{
                    color: ev.type === 'SERVICE_DOWN' ? C.red : C.accent,
                    minWidth: 120
                  }}
                >
                  [{ev.type || ev.stage || 'EVENT'}]
                </span>
                <span style={{ color: C.amber, minWidth: 110 }}>
                  {ev.service || ev.service_name || ''}
                </span>
...
function MetricsTab ({ services }) {
  ...
  useEffect(() => {
    if (names.length && !sel2) setSel2(names[0])
  }, [names.length])
  useEffect(() => {
    if (!sel2) return
    const load = () =>
      axios
        .get(`${API}/dashboard/metrics`, {
          params: { service: sel2, limit: 60 }
        })
        .then(r => setMetrics((r.data || []).slice().reverse()))
        .catch(() => {})
    load()
  ...
  const data = metrics.map(m => ({
    ...
    memory: parseFloat(m.memory) || 0,
    latency: parseFloat(m.response_time) || 0,
    ...
    <div>
      ...
      >
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 24 }}>
          {[
            { label: 'CPU', val: `${fmt(lat.cpu)}%`, color: C.accent },
            { label: 'MEM', val: `${fmt(lat.memory)}%`, color: C.purple },
            { label: 'P50', val: `${fmt(lat.latency, 0)}ms`, color: C.green },
            { label: 'ERR', val: fmt(lat.errors), color: C.red }
          ].map(x => (
              ...
              {data
                ...
                .map((m, i) => (
                  ...
                  >
                    ...
                    <td style={{ ...td, color: m.cpu > 80 ? C.red : C.text }}>
                      {fmt(m.cpu)}%
                    </td>
                    <td
                      style={{ ...td, color: m.memory > 80 ? C.red : C.text }}
                    >
                      {fmt(m.memory)}%
                    </td>
                    <td
                      style={{
                        ...td,
                        color: m.latency > 2000 ? C.amber : C.text
                      }}
                    >
                      {fmt(m.latency, 0)}
                    </td>
                    <td
                      style={{ ...td, color: m.errors > 0 ? C.red : C.muted }}
                    >
                      {fmt(m.errors)}
                    </td>
...
function TopologyTab ({ services }) {
  const sMap = {}
  services.forEach(s => {
    sMap[s.name || s.service_name] = s
  })
  const nc = name => {
    const s = sMap[name]
    return s ? statusColor(s.status) : C.muted
  }
  ...
  const makeNode = (id, label, x, y) => ({
    ...
    data: {
      label: (
        <div style={{ textAlign: 'center', fontSize: 10 }}>
          <div style={{ fontSize: 14, marginBottom: 2, color: nc(label) }}>
            ●
          </div>
          <div style={{ fontWeight: 700, fontSize: 11 }}>{label}</div>
          {sMap[label] && (
            <div style={{ fontSize: 9, color: '#aaa', marginTop: 2 }}>
              CPU {fmt(sMap[label].cpu)}% · MEM {fmt(sMap[label].memory)}%
            </div>
          )}
        </div>
      )
    ...
    style: {
      background: C.panel,
      border: `2px solid ${nc(label)}`,
      color: C.text,
      borderRadius: 8,
      padding: '10px 14px',
      fontSize: 12,
      width: 150,
      boxShadow: `0 0 16px ${nc(label)}33`
    }
  })
  ...
  return (
    ...
    >
      {diagnoses.length === 0 ? (
        ...
        diagnoses.map(d => {
          const sev = parseFloat(d.severity) || 0,
            col = sevColor(sev)
          return (
            <div
              key={d.service_name}
              style={{ ...pnl(), borderLeft: `3px solid ${col}` }}
            >
              <div
                style={{
                  display: 'flex',
              ...
              >
                <div>
                  <p
                    style={{ margin: '0 0 8px', fontSize: 14, fontWeight: 700 }}
                  >
                    {d.service_name}
                  </p>
                  <span style={badge(aColor(d.anomaly_type))}>
                    {d.anomaly_type || 'UNKNOWN'}
                  </span>
                </div>
              ...
              >
                {timeAgo(d.created_at || d.updated_at)}
              </p>
            </div>
          )
        })
...
function HealingTab ({ diagnoses, services }) {
  const allNames = [
    ...new Set(
      [
        ...diagnoses.map(d => d.service_name),
        ...services.map(s => s.name || s.service_name)
      ].filter(Boolean)
    )
  ]
  ...
  useEffect(() => {
    if (allNames.length && !sel2) setSel2(allNames[0])
  }, [allNames.length])
  useEffect(() => {
    if (!sel2) return
    const load = () =>
      axios
        .get(`${API}/history/${sel2}`)
  ...
  return (
    <div>
      ...
      {diag && (
        <div
          style={{ ...pnl(`${sevColor(diag.severity)}44`), marginBottom: 20 }}
        >
          ...
          <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap' }}>
            <div>
              ...
              <span style={badge(aColor(diag.anomaly_type))}>
                {diag.anomaly_type}
              </span>
            </div>
            <div>
              ...
              <p
                style={{
                  ...
                  fontSize: 24,
                  fontWeight: 700,
                  color: sevColor(diag.severity)
                }}
              >
                {fmt(parseFloat(diag.severity), 0)}%
              </p>
            </div>
            {diag.root_cause && (
        ...
        {history.length === 0 ? (
          ...
          <div style={{ overflowX: 'auto' }}>
            ...
            >
              <thead>
                <tr>
                  {['Time', 'Anomaly Type', 'Action', 'Status', 'Message'].map(
                    h => (
                      <th key={h} style={th}>
                        {h}
                      </th>
              ...
              <tbody>
                {history.slice(0, 40).map((h, i) => (
                  ...
                  >
                    ...
                    <td style={td}>
                      <span style={badge(aColor(h.anomaly_type))}>
                        {h.anomaly_type || '—'}
                      </span>
                    </td>
                    <td style={{ ...td, color: C.accent, fontSize: 11 }}>
                      {h.action_type}
                    </td>
                    <td style={td}>
                      <span
                        style={badge(
                          h.action_status === 'SUCCESS'
                            ? C.green
    ...
    >
      {all.length === 0 ? (
        <div style={pnl()}>
          ...
          >
            ✓ No predictive alerts. All services trending healthy.
          </p>
        </div>
      ) : (
        all.map((a, i) => {
            ...
            <div key={i} style={{ ...pnl(), borderTop: `3px solid ${col}` }}>
              ...
              >
                ...
                <div style={{ textAlign: 'right' }}>
                  ...
                  >
                    {parseFloat(a.ttl_minutes || 0).toFixed(1)}
                  </p>
                  <p
                    style={{
                      margin: 0,
                  ...
                  >
                    ...
                    <div style={{ fontSize: 11 }}>
                      ...
                      <span style={{ color: col }}>
                        {fmt(parseFloat(f.forecast || 0), 1)}
                      </span>
                      <span style={{ color: C.muted }}>
                        {' '}
                        in {f.minutesToSLA}min
                      </span>
                    </div>
                  </div>
                ))}
...
function LogsTab ({ liveEvents }) {
  ...
  const lCol = l =>
    ({ ERROR: C.red, WARN: C.amber, INFO: C.accent }[l] || C.muted)
    ...
    <div>
      ...
      >
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 10 }}>
          <span style={badge(C.red)}>
            ERR {logs.filter(l => l.level === 'ERROR').length}
          </span>
          <span style={badge(C.amber)}>
            WARN {logs.filter(l => l.level === 'WARN').length}
          </span>
          <span style={badge(C.accent)}>
      ...
      {liveEvents.length > 0 && (
        <div style={{ ...pnl(), marginBottom: 16 }}>
          <SectionTitle
            title='LIVE STREAM'
            extra={<span style={badge(C.green)}>SOCKET</span>}
          />
        ...
        <div style={{ ...logBox, maxHeight: 500 }}>
          {logs.length === 0 ? (
            ...
            logs.map((l, i) => (
              ...
              >
                <span style={{ color: C.muted, minWidth: 72, fontSize: 10 }}>
                  {l.created_at
                ...
                <span style={{ color: C.amber, minWidth: 120, fontSize: 10 }}>
                  {l.service_name}
                </span>
